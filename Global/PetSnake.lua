...
--(you can equip two other hats that will go on the snakes heads)
...
Colliders = Instance.new("Folder",Workspace)
Colliders.Name = "Colliders"

function brick(Accessory)
    Accessory.Handle.AccessoryWeld:Destroy()
    collision = Instance.new("Part",Colliders)
    collision.Position = Accessory.Handle.Position
    collision.Size = Accessory.Handle.Size
    collision.Transparency = 1
    collision.Name = Accessory.Name
    collweld = Instance.new("Weld",collision)
    collweld.Part0 = collision
    collweld.Part1 = Accessory.Handle
    for i,v in pairs(char.Model[Accessory.Name].Handle:GetDescendants()) do
        if v:IsA("SpecialMesh") then v:Destroy() end
    end
    Accessory.Name = "USED"
end
function socket(Part,X,Y,Z)
    attach = Instance.new("Attachment",Part)
    attach.CFrame = CFrame.new(X,Y,Z)
    attach.Name = "Start"
    attach = Instance.new("Attachment",Part)
    attach.CFrame = CFrame.new(X*-1,Y*-1,Z*-1)
    attach.Name = "End"
end
function join(Part1,Part2)
    joint = Instance.new("BallSocketConstraint",Part1)
    joint.Attachment0 = Part1.End
    joint.Attachment1 = Part2.Start
end

brick(char.Hat1)
socket(Colliders.Hat1,0,0,1)

brick(char.LavanderHair)
socket(Colliders.LavanderHair,0,0,1)

brick(char.Robloxclassicred)
socket(Colliders.Robloxclassicred,0,0,1)

join(Colliders.Hat1,Colliders.LavanderHair)
join(Colliders.LavanderHair,Colliders.Robloxclassicred)


brick(char.Necklace)
socket(Colliders.Necklace,0,0,1)

brick(char["Kate Hair"])
socket(Colliders["Kate Hair"],0,0,1)

brick(char["Pal Hair"])
socket(Colliders["Pal Hair"],0,0,1)

join(Colliders.Necklace,Colliders["Kate Hair"])
join(Colliders["Kate Hair"],Colliders["Pal Hair"])


brick(char.InternationalFedora)
socket(Colliders.InternationalFedora,0,0,0.5)

join(Colliders.Robloxclassicred,Colliders.InternationalFedora)

brick(char["International Fedora"])
socket(Colliders["International Fedora"],0,0,0.5)

join(Colliders["Pal Hair"],Colliders["International Fedora"])


Character = Instance.new("Model",Colliders)
Character.Name = "snek1"

HumanoidRootPart = Instance.new("Part",Character)
HumanoidRootPart.Position = char.Head.Position
...
Head = Colliders.InternationalFedora
Head.Parent = Character
Head.Name = "Head"
Head.Transparency = 1
for i,v in pairs(char:GetDescendants()) do
    if v:IsA("Accessory") and v.Name ~= "USED" and v.Parent == char then
        v.Handle.AccessoryWeld.Part1 = Head
        v.Name = "USED"
        break
    end
end
...
RootJoint.MaxVelocity = 1


Humanoid1 = Instance.new("Humanoid",Character)
Humanoid1.HipHeight = 0.2
--##################################
Character = Instance.new("Model",Colliders)
Character.Name = "snek2"
...
Head = Colliders["International Fedora"]
Head.Parent = Character
Head.Name = "Head"
Head.Transparency = 1
for i,v in pairs(char:GetDescendants()) do
...
Humanoid2 = Instance.new("Humanoid",Character)
Humanoid2.HipHeight = 0.2

mouse = lp:GetMouse()

Move1 = Instance.new("Tool",lp.Backpack)
Move1.Name = "Snek One"
Move2 = Instance.new("Tool",lp.Backpack)
Move2.Name = "Snek Two"
Move3 = Instance.new("Tool",lp.Backpack)
Move3.Name = "Both"

mouse.Button1Up:connect(function()
    if Move1.Parent == char then Humanoid1:MoveTo(mouse.Hit.p) end
    if Move2.Parent == char then Humanoid2:MoveTo(mouse.Hit.p) end
end)
...
while task.wait() do
    if Move3.Parent == char then 
        Move3.Parent = lp.Backpack
        Move2.Parent = char
        Move1.Parent = char
    end
